...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
	...
	for itr in range(resume_itr, FLAGS.meta_train_iterations):
		...
		meta_batch = data_generator.sample_batch('meta_train', FLAGS.meta_batch_size)
		inputa = meta_batch[0][:, :, :FLAGS.k_shot, :]
		labela = meta_batch[1][:, :, :FLAGS.k_shot, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
		inputb = meta_batch[0][:, :, FLAGS.k_shot:, :]
		labelb = meta_batch[1][:, :, FLAGS.k_shot:, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
		...
		if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0:
		    ...
		    # group a (inputa, labela) and group b (inputb, labelb)
			meta_batch = data_generator.sample_batch('meta_val', FLAGS.meta_batch_size)
			inputa = meta_batch[0][:, :, :FLAGS.k_shot, :]
			labela = meta_batch[1][:, :, :FLAGS.k_shot, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
			inputb = meta_batch[0][:, :, FLAGS.k_shot:, :]
			labelb = meta_batch[1][:, :, FLAGS.k_shot:, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
			...
			accuracies_plot.append(result[-1])

	train_writer.close()

	plt.plot(list(range(TEST_PRINT_INTERVAL,FLAGS.meta_train_iterations,TEST_PRINT_INTERVAL))[:len(accuracies_plot)], accuracies_plot)
	plt.show()
	with open(FLAGS.logdir + '/' + exp_string + '.txt', 'w') as f:
		for item in accuracies_plot:
			f.write("%s\n" % item)

	saver.save(sess, FLAGS.logdir + '/' + exp_string +  '/model' + str(FLAGS.meta_train_iterations))
...
def meta_test(model, saver, sess, exp_string, data_generator, meta_test_num_inner_updates=None):
	...
	for _ in range(NUM_META_TEST_POINTS):
		...
		# group a (inputa, labela) and group b (inputb, labelb)
		meta_batch = data_generator.sample_batch('meta_test', FLAGS.meta_batch_size)
		inputa = meta_batch[0][:, :, :FLAGS.k_shot, :]
		labela = meta_batch[1][:, :, :FLAGS.k_shot, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
		inputb = meta_batch[0][:, :, FLAGS.k_shot:, :]
		labelb = meta_batch[1][:, :, FLAGS.k_shot:, :].reshape((FLAGS.meta_batch_size, -1, FLAGS.n_way))
