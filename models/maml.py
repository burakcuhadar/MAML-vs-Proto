...
class MAML:
	...
	def construct_model(self, prefix='maml'):
		...
		with tf.variable_scope('model', reuse=None) as training_scope:
			...
			# Construct inner update learning rate variables
			# TODO weights'teki gibi dir(self)'in içinde mi diye kontrol etmek gerekmiyor mu?
			if FLAGS.learn_inner_update_lr:
				self.inner_update_lrs = inner_update_lrs = self.construct_inner_lrs()
			...
			def task_inner_loop(inp, reuse=True):
				...
				inner_weights = dict(zip(weights.keys(),
										 [tf.Variable(lambda: weights[key].read_value()) for key in weights.keys()]))

				# output, loss, and accuracy of group a before performing inner gradient update
				task_outputa = self.forward_conv(inputa, inner_weights, reuse=tf.AUTO_REUSE)
				task_lossa = self.loss_func(task_outputa, labela)
				total = tf.reduce_sum(tf.cast(tf.equal(tf.argmax(labela, 1), tf.argmax(task_outputa, 1)), tf.float32))
				task_accuracya = total / tf.cast(tf.shape(labela)[0], tf.float32)
				...
				for i in range(num_inner_updates):
					output = self.forward_conv(inputa, inner_weights, reuse=tf.AUTO_REUSE)
					loss = self.loss_func(output, labela)

					for key in inner_weights:
						if FLAGS.learn_inner_update_lr:
							inner_weights[key] -= inner_update_lrs[key][i] * tf.gradients(loss, inner_weights[key])[0]
						else:
							inner_weights[key] -= self.inner_update_lr * tf.gradients(loss, inner_weights[key])[0]

					# Performance after gradient update
					task_outputb = self.forward_conv(inputb, inner_weights, reuse=tf.AUTO_REUSE)
					task_lossb = self.loss_func(task_outputb, labelb)
					task_accuracyb = tf.reduce_sum(tf.cast(tf.equal(tf.argmax(labelb, 1), tf.argmax(task_outputb, 1)), tf.float32)) / tf.cast(tf.shape(labelb)[0], tf.float32)
	...
	def construct_inner_lrs(self):
		inner_lrs = {}
		dtype = tf.float32
		num_inner_updates = max(self.meta_test_num_inner_updates, FLAGS.num_inner_updates)

		inner_lrs['conv1'] = tf.get_variable('conv1_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['b1'] = tf.get_variable('b1_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['conv2'] = tf.get_variable('conv2_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['b2'] = tf.get_variable('b2_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['conv3'] = tf.get_variable('conv3_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['b3'] = tf.get_variable('b3_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['conv4'] = tf.get_variable('conv4_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['b4'] = tf.get_variable('b4_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['w5'] = tf.get_variable('w5_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
		inner_lrs['b5'] = tf.get_variable('b5_lr', initializer=[self.inner_update_lr] * num_inner_updates, dtype=dtype)
