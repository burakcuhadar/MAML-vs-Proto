...
if __name__ == '__main__':
	...
	for epi in range(n_meta_test_episodes):
        ...
        # support and query sets
		meta_batch = data_generator.sample_batch('meta_test', 1, shuffle=False)
		support = np.reshape(meta_batch[0][0, :, :k_meta_test_shot, :], (n_meta_test_way, k_meta_test_shot, im_height, im_width, channels))
		query = np.reshape(meta_batch[0][0, :, k_meta_test_shot:, :], (n_meta_test_way, n_meta_test_query, im_height, im_width, channels))
		labels = meta_batch[1][0, :, k_meta_test_shot:, :]
